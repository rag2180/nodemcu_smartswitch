...
void setup() {
  ...
  server.on("/check_server", check_server);
  server.begin();
...
void check_server() {
  Serial.println("Server Working Fine");
  server.send(200, "text/html", "Ok Working. Local Server is FINE.");
}
...
void loop() {
  ...
  Serial.println("Looping...Wifi Status - ");
  Serial.println(WiFi.status());
  if (WiFi.status() == WL_CONNECTED) { //Check WiFi connection status
    ...
    if (httpCode > 0) { //Check the returning code
      ...
      http.end();
      Serial.println("Unable to Fetch Required State - httpcode - ");
